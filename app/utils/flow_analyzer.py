...
def time_flow(PCAPS):
    ...
    time_flow_dict[time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(start))] = len(corrupt_bytes(PCAPS[0]))
    for pcap in PCAPS:
        pcap_time_as_float = float(pcap.time)
        timediff = pcap_time_as_float - start
        time_flow_dict[time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(pcap_time_as_float))] = len(corrupt_bytes(pcap))
